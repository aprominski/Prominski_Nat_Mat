#### Module provided by Intan technologies under GPL 3.0 license.
#### https://github.com/Intan-Technologies/load-rhd-notebook-python
####
#### Function get_tV was introduced to load time (t) and voltage (V) vectors from specific channels.
...
def get_tV(channel_name, result):
    channel_found, signal_type, signal_index = find_channel_in_header(channel_name, result)
    if channel_found:
        if signal_type == 'amplifier_channels':
            ylabel = 'Voltage (microVolts)'
            ...
            ylabel = 'Voltage (Volts)'
            signal_data_name = 'aux_input_data'
            time = result['t_aux_input']
        
        elif signal_type == 'supply_voltage_channels':
            ylabel = 'Voltage (Volts)'
            signal_data_name = 'supply_voltage_data'
            time = result['t_supply_voltage']
         
            
        elif signal_type == 'board_adc_channels':
            ylabel = 'Voltage (Volts)'
            signal_data_name = 'board_adc_data'
            time = result['t_board_adc']
            ...
            print('channel is not amplifier')
